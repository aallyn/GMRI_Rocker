...
vast_make_extrap_grid<- function(region_shapefile, index_shapes, strata.limits, cell_size){
  ...
  # For debugging
  if(FALSE){
    tar_load(region_shapes)
    index_shapes = region_shapes
    strata.limits = c("California Current Large Marine Ecosystem", "Northeast U.S. Continental Shelf Large Marine Ecosystem")
    cell_size = 25000
  }
...
vast_make_settings <- function(extrap_grid, FieldConfig, RhoConfig, OverdispersionConfig, bias.correct, Options, strata.limits){
  ...
  settings_out<- make_settings(n_x = n_x_use, Region = "User", purpose = "index2", FieldConfig = FieldConfig, RhoConfig = RhoConfig, ObsModel = c(2, 1), OverdispersionConfig = OverdispersionConfig, bias.correct = bias.correct, knot_method = "grid", treat_nonencounter_as_zero = FALSE, strata.limits = strata.limits)
